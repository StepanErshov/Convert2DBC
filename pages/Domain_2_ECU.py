...
if uploaded_file:
    try:
        ...
        if not bus_users:
            st.error("🕭 No ECUs found in the file. Please check that the file contains columns with 'S' or 'R' values.")
            st.stop()
        ...
        if selected_ecus and st.button("🖊 Export selected ECUs"):
            ...
            with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
                for selected_ecu in selected_ecus:
                    ...
                    if not history_found:
                        st.warning(f"🕱 'History' sheet not found in ECU {selected_ecu}.")
            ...
            st.download_button(
                label=f"🖥 Download all files ({len(selected_ecus)} ECUs)",
                data=zip_buffer,
                file_name=zip_filename,
                mime="application/zip",
                key="download_zip"
            )
            st.success(f"Export completed for {len(selected_ecus)} ECUs. All files are ready for download in one archive.")
