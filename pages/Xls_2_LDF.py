...
def validate_input_data(uploaded_file) -> Tuple[List[str], List[str]]:
    ...
    try:
        ...
        if len(df_info.columns) < 4:
            errors.append("Info sheet must have at least 4 columns with configuration data")
            return errors, warnings

        version_str = str(df_info.iloc[1, 0]).strip(".")
        if not re.match(r"^\d\.\d$", version_str):
            errors.append(f"Invalid LIN version format '{version_str}'. Must be in format X.Y")

        try:
            baudrate = float(df_info.iloc[1, 1]) * 1000
            if not (1000 <= baudrate <= 20000):
                warnings.append(f"Baudrate {baudrate} is outside typical LIN range (1-20 kbps)")
        ...
        missing_columns = [col for col in required_matrix_columns if col not in df_matrix.columns]
        if missing_columns:
            errors.append(f"Matrix sheet missing required columns: {', '.join(missing_columns)}")
            return errors, warnings
        ...
        for msg_id in df_matrix["Msg ID(hex)\n报文标识符"].dropna().unique():
            try:
                ...
                if not (0 <= frame_id <= 0x3F):
                    errors.append(f"Invalid LIN frame ID {hex(frame_id)}. Must be 0x00-0x3F")
                
                if frame_id in seen_ids:
                    errors.append(f"Duplicate frame ID {hex(frame_id)}")
                seen_ids.add(frame_id)
            except ValueError:
                errors.append(f"Invalid frame ID format '{msg_id}'. Must be hex (e.g. 0x12)")
        ...
        try:
            ...
            if not df_schedule.empty:
                for col in df_schedule.columns:
                    ...
                    delay_col = df_schedule.columns[df_schedule.columns.get_loc(col) + 2]
