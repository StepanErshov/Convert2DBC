...
for (msg_id, msg_name), group in grouped:
    try:
        ...
        for _, row in group.iterrows():
            try:
                byte_order = (
                    "motorola" if row["Byte Order"] == "Motorola MSB" else "intel"
                )
                ...
                if pd.notna(row["Signal Value Description"]):
                    value_descriptions = parse_value_descriptions(
        ...
        for signal in signals:
            ...
            for bit in range(start, end):
                if bit in used_bits:
                    print(
                        f"Предупреждение: Перекрытие битов в сообщении {msg_name} (0x{frame_id:X}), сигнал {signal.name} (бит {bit})"
                    )
                    overlap_found = True
                    break
                used_bits.add(bit)
        ...
        if overlap_found:
            print(
                f"Пропускаю сообщение {msg_name} (0x{frame_id:X}) из-за перекрытия битов"
            )
            continue
